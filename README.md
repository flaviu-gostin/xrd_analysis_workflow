...
- Create a virtual environment, e.g. install virtualenv with `sudo apt install virtualenv` if it is not already installed, create a v env with `virtualenv --python=python3.5 venv` (a directory `venv/` will be created, which will contain the Python packages)
- Activate the v env with `source venv/bin/activate` (deactivate it with `deactivate`) 
- Install the requirements in `requirements.txt`, e.g. with `pip install -r requirements.txt`
- Dig deep inside the v env directory for the module pyFAI.azimuthalIntegrator (something like `xrd_analysis_workflow/venv/lib/python3.5/site-packages/pyFAI/azimuthalIntegrator.py) and insert `try:` above `from .opencl import ocl`, of course indent the latter, and below it add two lines `except ImportError:` and with indent `ocl = None` (see Important note below)
- Finally, download the raw diffraction data from Figshare and do the analysis simply with the command (make sure you are in the project root, i.e. `xrd_analysis_workflow` and the v env is active): `make all`
- If you get some ImportError warnings, you probably need to install those modules that are indicated in the warnings. Make sure the v env is active and install those modules with `pip install module_name`
...
- Finally finally, compare the `xrd_analysis_workflow/results/final/ directory` as said above.
